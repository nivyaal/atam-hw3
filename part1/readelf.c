	...
	printf ("%s\n      ", printable_section_name (filedata, section));
      else
	  {
		  if ( section->sh_type == SHT_NULL)
		  {
			...
			char* kirel= "_<3KIREL";
		  	char* section_name = SECTION_NAME_PRINT (section);
		  	int size= strlen(section_name);
		  	char* s=(char*)malloc((size+8)*sizeof(char));
		  	strcpy(s,section_name);
			strcpy(s+size, kirel);
			print_symbol(-17, s);
			free(s);
		  }
	   ...
	   i++, section++)
	{
	  ...
	  for (si = 0; si < num_syms; si++)
	  {
	    print_dynamic_symbol (filedata, si, symtab, section,
				  strtab, strtab_size);
		if(!strcmp(get_symbol_binding (filedata, ELF_ST_TYPE (ELF_ST_BIND((symtab + si)->st_info))), "GLOBAL"))
	 		cnt++;
	  }
	  if (cnt == 0)
